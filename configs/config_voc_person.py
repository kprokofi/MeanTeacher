...
def get_config_voc_person():
    ...
    C.dataset_path = osp.join(C.volna, 'pascal_voc/person')
    ...
    with open(C.unsup_source) as f:
        all_im = f.readlines()
        C.num_unsup_imgs = len(all_im)     # unsupervised samples
    ...
    C.area_thresh = 0.03
    C.area_thresh2 = 0.03
    C.criterion = dict(cons=dict(sample=True, gamma=2),
