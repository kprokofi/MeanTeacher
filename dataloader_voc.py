...
import random
from configs import config_voc, config_city
...
class ValPre(object):
    ...
    def __call__(self, img, gt):
        ...
        img = cv2.resize(img, (self.config.image_height, self.config.image_width), interpolation=cv2.INTER_LINEAR)
        gt = cv2.resize(gt, (self.config.image_height, self.config.image_width), interpolation=cv2.INTER_LINEAR)

        return img, gt, extra_dict

def get_train_loader_voc(engine, dataset, train_source, config, unsupervised=False, collate_fn=None):
...
class VOC(BaseDataset):
    def __init__(self, setting, split_name, preprocess=None,
                 file_length=None, training=True, unsupervised=False, pseudo_label=False, **kwargs):
